  ...
  async handleRequest(req: ActivistRequest): Promise<AppState> {
    ...
    while (state.iterations < MAX_ITERATIONS) {
      ...
      if (score >= JUDGE_THRESHOLD) {
        ...
        state.memory_kv.improve_instruction =
          `Оценка ${score.toFixed(2)} — недостаточно конкретно. Добавь: конкретные порталы и ссылки, пошаговый алгоритм действий, сроки и статьи законов.`
      }
    }
    ...
    this.sql`UPDATE sessions SET status=${state.status}, category=${state.category || "unknown"},
      response=${state.final_response || ""}, iterations=${state.iterations},
